...
class Page extends LitElement {
	...
	private _importCreateStorageFilesTask = new Task(this, {
		task: async ([_showUploadNewStorageFiles]) => {
			if (this._createStorageFilesImported || !_showUploadNewStorageFiles) {
				return
			}
		...
		args: () => [this._showUploadNewStorageFiles]
	})
					...
					if (this._showUploadNewStorageFiles) {
						return this._importCreateStorageFilesTask.render({
							...
							complete: () => html`
								...
								<create-storage-files class="flex-[9.5]"></create-storage-files>
							`,
							error: (e) => {
								console.error(e)
								return html`
