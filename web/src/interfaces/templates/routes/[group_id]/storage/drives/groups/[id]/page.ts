...
@customElement('storage-drive-group')
class Page extends LitElement {
	...
	connectedCallback(): void {
		...
		if (this.data && this.data.metadata_model && this.data.datum) {
			let value = MetadataModel.DatabaseGetColumnFieldValue(this.data.metadata_model, Entities.StorageDrivesGroups.FieldColumn.Description, this.data.metadata_model[MetadataModel.FgProperties.DATABASE_TABLE_COLLECTION_UID], this.data.datum)
	...
	private async _handleUpdateStorageDrivesGroups() {
		if (!this.data || !this.data.datum) {
			return
		}

		let data: Entities.StorageDrivesGroups.Interface = {
			storage_drives_id: (this.data.datum as Entities.StorageDrivesGroups.Interface).storage_drives_id,
			directory_groups_id: (this.data.datum as Entities.StorageDrivesGroups.Interface).directory_groups_id
		}
		if (!Json.AreValuesEqual([this._description], (this.data.datum as Entities.StorageDrivesGroups.Interface).description)) {
			data.description = [this._description]
		}

		this.dispatchEvent(new CustomEvent(Lib.CustomEvents.SHOW_LOADING_SCREEN, { detail: { loading: true, loadingMessage: `Updating ${Entities.StorageDrivesGroups.RepositoryName}` }, bubbles: true, composed: true }))
		try {
			if (!this._appContext.GetCurrentdirectorygroupid()) {
				return
			}
			const fetchUrl = new URL(`${Url.ApiUrlPaths.Storage.Drives.Groups}/${Url.Action.UPDATE}`)
			...
			if (fetchResponse.ok) {
				...
				if (fetchData.successful && fetchData.successful > 0 && !fetchData.failed) {
					if (!Json.AreValuesEqual([this._description], (this.data.datum as Entities.StorageDrivesGroups.Interface).description)) {
						;(this.data.datum as Entities.StorageDrivesGroups.Interface).description = [this._description]
					}
				}
			} else {
	...
	private async _handleDeleteStorageDrivesGroups() {
		if (!this.data || !this.data.datum) {
			return
		}

		this.dispatchEvent(new CustomEvent(Lib.CustomEvents.SHOW_LOADING_SCREEN, { detail: { loading: true, loadingMessage: `Deleting ${Entities.StorageDrivesGroups.RepositoryName}` }, bubbles: true, composed: true }))
		try {
			if (!this._appContext.GetCurrentdirectorygroupid()) {
				return
			}
				...
				if (fetchData.successful && fetchData.successful > 0 && !fetchData.failed) {
					try {
						...
						if (targetElement !== null) {
							...
							if (dgid) {
								let url = new URL(Url.WebsitePaths.Storage.Drives.Groups, window.location.origin)
								url.searchParams.append(Url.SearchParams.DIRECTORY_GROUP_ID, dgid)
								Url.AddBaseUrl(url)
								await this._pageNavigation.Navigate(targetElement, url, 'Storage Drives Groups')
							}
						}
					} catch (e) {
					...
					if (this._showCreateEdit) {
						return html`
							...
							<div class="join w-full">
								<button class="flex-[2] join-item btn btn-secondary flex flex-col justify-center" @click=${this._handleUpdateStorageDrivesGroups}>
									...
									${(() => {
										...
										return html`<div>Edit Storage Drive Group</div>`
									})()}
								</button>
								<button class="flex-1 join-item btn btn-secondary flex flex-col justify-center" @click=${this._resetFields}>
									...
									${(() => {
										...
										return html`<div>Reset Storage Drive Group</div>`
									})()}
								</button>
								${(() => {
									if (!this.data || !this.data.metadata_model || !this.data.datum) {
									...
									return html`
										<button class="flex-1 join-item btn btn-accent flex flex-col justify-center" @click=${() => (this._showCreateEdit = false)}>
											...
											${(() => {
												...
												return html`<div>Close Edit Storage Drive Group</div>`
											})()}
										</button>
									`
								})()}
						...
						return this._importMMViewDatumTask.render({
							...
							complete: () => html`
								...
								<div class="join w-full">
									...
									<button class="flex-1 join-item btn btn-secondary flex flex-col justify-center" @click=${this._handleDeleteStorageDrivesGroups}>
										...
										${(() => {
											...
											return html`<div>Delete Storage Drive Group</div>`
										})()}
									</button>
								</div>
							`,
