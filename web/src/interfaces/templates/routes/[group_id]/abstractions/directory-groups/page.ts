...
@customElement('abstractions-directory-groups')
class Page extends LitElement {
	...
	constructor() {
		...
		this._metadataModelsSearch = new MetadataModelSearchController(this, `${Url.ApiUrlPaths.Abstractions.DirectoryGroups}${Url.MetadataModelSearchGetMMPath}`, `${Url.ApiUrlPaths.Abstractions.DirectoryGroups}${Url.MetadataModelSearchPath}`)
	}
	...
	private async _handleDatabaseSearch() {
		...
		if (this._metadataModelsSearch.searchmetadatamodel) {
			...
			if (this._dateOfCreationFrom.length > 0) {
				newQc['$.created_on'] = {
					[MetadataModel.QcProperties.D_FIELD_COLUMN_NAME]: Entities.AbstractionsDirectoryGroups.FieldColumn.CreatedOn,
					[MetadataModel.QcProperties.D_TABLE_COLLECTION_UID]: this._metadataModelsSearch.searchmetadatamodel[MetadataModel.FgProperties.DATABASE_TABLE_COLLECTION_UID],
					[MetadataModel.QcProperties.FG_FILTER_CONDITION]: [
						[
							{
			...
			if (this._dateOfLastUpdatedOnFrom.length > 0) {
				newQc['$.last_updated_on'] = {
					[MetadataModel.QcProperties.D_FIELD_COLUMN_NAME]: Entities.AbstractionsDirectoryGroups.FieldColumn.LastUpdatedOn,
					[MetadataModel.QcProperties.D_TABLE_COLLECTION_UID]: this._metadataModelsSearch.searchmetadatamodel[MetadataModel.FgProperties.DATABASE_TABLE_COLLECTION_UID],
					[MetadataModel.QcProperties.FG_FILTER_CONDITION]: [
						[
							{
	...
	private async _handleDeleteAbstractionsDirectoryGroups(selectedDataIndexes: number[]) {
		const data = selectedDataIndexes.map((dIndex) => this._metadataModelsSearch.searchresults.data![dIndex])

		try {
			this.dispatchEvent(new CustomEvent(Lib.CustomEvents.SHOW_LOADING_SCREEN, { detail: { loading: true, loadingMessage: `Deleting/deactivating ${Entities.AbstractionsDirectoryGroups.RepositoryName}...` }, bubbles: true, composed: true }))
		...
		if (action) {
			switch (action) {
				case Url.Action.CREATE:
					this._handlePageNavigation(`${Url.WebsitePaths.Abstractions.DirectoryGroups}/new`, 'New Abstractions Directory Groups')
					break
				...
				<header class="flex-[0.5] flex flex-col gap-y-1 z-[2]">
					<section class="join w-[50%] rounded-md self-center border-[1px] border-primary p-1">
						<input
							...
							type="search"
							placeholder="Search abstractions directory groups..."
												...
												<div class="grow-[9] border-[1px] border-gray-400 rounded-md h-fit max-h-full max-w-full flex overflow-hidden">
													<metadata-model-view-table
														...
														@metadata-model-view-table:rowclick=${async (e: CustomEvent) => {
															const datum = e.detail.value as Entities.AbstractionsDirectoryGroups.Interface
															if (Array.isArray(datum.directory_groups_id) && datum.directory_groups_id.length == 1) {
																this._handlePageNavigation(`${Url.WebsitePaths.Abstractions.DirectoryGroups}/${datum.directory_groups_id[0]}`, `Abstractions Directory Groups: ${datum.directory_groups_id[0]}`)
															}
														}}
														.addselectcolumn=${true}
														.selecteddataindexesactions=${[
															{
																actionName: 'Delete/deactivate selected abstractions directory groups',
																action: (selectedDataIndexes: number[]) => {
																	this._handleDeleteAbstractionsDirectoryGroups(selectedDataIndexes)
																}
															}
														]}
													></metadata-model-view-table>
								...
								<section class="shrink-[9] overflow-auto flex justify-center bg-gray-300 rounded-md w-full h-full min-h-[100px]">
									<div class="self-center flex-1 flex flex-col max-md:max-w-[80%] min-w-fit min-h-fit gap-y-10">
										${(() => {
											...
											return html` <div class="text-xl font-bold break-words text-center">${Url.abstractionsDirectoryGroupsNavigation.description}</div> `
										})()}
										<div class="flex justify-evenly flex-wrap gap-8">
											...
											>
												...
												${(() => {
													...
													return html`<div>Create new abstractions directory groups</div>`
												})()}
											</button>
											<button class="link link-hover min-h-fit h-fit min-w-fit w-fit flex flex-col justify-center" @click=${() => (this._showQueryPanel = true)}>
												...
												${(() => {
													...
													return html`<div>Search for abstractions directory groups</div>`
												})()}
											</button>
											...
											<button class="link link-hover min-h-fit h-fit min-w-fit w-fit flex flex-col justify-center" @click=${() => (this._showQueryPanel = true)}>
												...
												${(() => {
													...
													return html`<div>Update abstractions directory groups</div>`
												})()}
											</button>
											<button class="link link-hover min-h-fit h-fit min-w-fit w-fit flex flex-col justify-center" @click=${() => (this._showQueryPanel = true)}>
												<div class="flex gap-x-1 self-center">
...
declare global {
	interface HTMLElementTagNameMap {
		'abstractions-directory-groups': Page
	}
}
