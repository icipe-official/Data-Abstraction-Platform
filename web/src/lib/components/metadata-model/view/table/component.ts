...
class Component extends LitElement {
	...
	private _rowColumnDataHtmlTemplate(rowIndex: number, columnIndex: number, stickyleft: number, stickytop: number) {
		return keyed(`${rowIndex}-${columnIndex}`, html`<div>${rowIndex}-${columnIndex}</div>`)
	}
	...
	@state() private _tableInsideTable!: boolean
	connectedCallback(): void {
		super.connectedCallback()

		this._tableInsideTable = typeof this.scrollelement !== 'undefined'
	...
	protected render(): unknown {
		...
		return html`
			<div id="scroll-element" class="flex flex-col flex-grow ${!this._tableInsideTable ? 'overflow-auto' : 'w-fit h-fit'}">
				${(() => {
					...
					return html`
						<header
							id="top-header"
							class="rounded-t-md h-fit min-w-fit sticky top-0 left-0 right-0 text-sm font-bold z-50 flex flex-col ${this.color === Theme.Color.PRIMARY
						...
						${(() => {
							...
							return html`
								<main class="relative grid min-h-fit min-w-fit w-fit h-fit rounded-md overflow-visible" style="grid-template-columns: repeat(${this._lockedColumnData2DFieldsIndex.length + (this._unlockedColumnEndIndex + 1 - this._unlockedColumnStartIndex) + 3}, minmax(min-content,500px));">
									<header
										id="column-header"
										...
										class="grid h-fit min-w-fit sticky left-0 right-0 shadow-sm text-sm font-bold z-40 shadow-gray-800 ${this.color === Theme.Color.PRIMARY
											? 'bg-primary text-primary-content'
											: this.color === Theme.Color.SECONDARY
												? 'bg-secondary text-secondary-content'
												: 'bg-accent text-accent-content '}"
