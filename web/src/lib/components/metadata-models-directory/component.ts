...
class Component extends LitElement {
	...
	constructor() {
		...
		this._metadataModelsSearch = new MetadataModelSearchController(this, `${Url.ApiUrlPaths.MetadataModels.Directory.Url}${Url.MetadataModelSearchGetMMPath}`, `${Url.ApiUrlPaths.MetadataModels.Directory.Url}${Url.MetadataModelSearchPath}`)
	}
	...
	private async _handleCreateUpdateMMDGs() {
		...
		let newMMDGs: Entities.MetadataModelsDirectory.Interface[] = []
		...
		try {
			this.dispatchEvent(new CustomEvent(Lib.CustomEvents.SHOW_LOADING_SCREEN, { detail: { loading: true, loadingMessage: `${this._createMMDGs ? 'Creating' : 'Updating'} ${Entities.MetadataModelsDirectory.RepositoryName}...` }, bubbles: true, composed: true }))
			if (!this._appContext.GetCurrentdirectorygroupid()) {
				return
			}
			const fetchUrl = new URL(`${Url.ApiUrlPaths.MetadataModels.Directory.Url}/${this._createMMDGs ? Url.Action.CREATE : Url.Action.UPDATE}`)
					...
					if (this._showCreateEditMMDGs) {
						return html`
							<header class="flex-[0.5] flex gap-x-1 z-[2]">
								...
								<div class="h-fit w-fit flex gap-x-1 text-primary self-center">${this._createMMDGs ? 'Create' : 'Update'} Metadata-Models Directory</div>
							</header>
							<nav class="w-fit steps self-center h-fit z-[1]">
								...
								>
									<div class="break-words pl-4 pr-4">${this._createMMDGs ? 'Create' : 'Update'} Metadata-Models Directory</div>
								</button>
							</nav>
														...
														<div class="grow-[9] border-[1px] border-gray-400 rounded-md h-fit max-h-full max-w-full flex overflow-hidden">
															<metadata-model-view-table
																...
																.selecteddataindexesactions=${[
																	{
																		actionName: 'Delete selected metadata-models directory',
										...
										<section class="shrink-[9] overflow-auto flex justify-center bg-gray-300 rounded-md w-full h-full min-h-[100px]">
											<div class="self-center flex-1 flex flex-col max-md:max-w-[80%] min-w-fit min-h-fit gap-y-10">
												${(() => {
													...
													return html` <div class="text-xl font-bold break-words text-center">${Url.metadataModelsDirectoryNavigation.description}</div> `
												})()}
												<div class="flex justify-evenly flex-wrap gap-8">
													...
													>
														<div class="flex gap-x-1 self-center">
															...
															<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24"><path d="M12 4a4 4 0 0 1 4 4a4 4 0 0 1-4 4a4 4 0 0 1-4-4a4 4 0 0 1 4-4m0 10c4.42 0 8 1.79 8 4v2H4v-2c0-2.21 3.58-4 8-4" /></svg>
														...
														${(() => {
															...
															return html`<div>Create new metadata-models directory</div>`
														})()}
													</button>
													<button class="link link-hover min-h-fit h-fit min-w-fit w-fit flex flex-col justify-center" @click=${() => (this._showQueryPanel = true)}>
														...
														${(() => {
															...
															return html`<div>Search for metadata-models directory</div>`
														})()}
													</button>
													...
													>
														...
														${(() => {
															...
															return html`<div>Update metadata-models directory</div>`
														})()}
													</button>
													<button class="link link-hover min-h-fit h-fit min-w-fit w-fit flex flex-col justify-center" @click=${() => (this._showQueryPanel = true)}>
														<div class="flex gap-x-1 self-center">
														...
														${(() => {
															...
															return html`<div>Delete metadata-models directory</div>`
														})()}
													</button>
												</div>
											</div>
...
declare global {
	interface HTMLElementTagNameMap {
		'metadata-models-directory': Component
	}
}
