...
@customElement('storage-drives-groups')
class Page extends LitElement {
	...
	constructor() {
		...
		this._metadataModelsSearch = new MetadataModelSearchController(this, `${Url.ApiUrlPaths.Storage.Drives.Groups}${Url.MetadataModelSearchGetMMPath}`, `${Url.ApiUrlPaths.Storage.Drives.Groups}${Url.MetadataModelSearchPath}`)
	}
	...
	private async _handleDatabaseSearch() {
		...
		if (this._metadataModelsSearch.searchmetadatamodel) {
			if (this._fullTextSearchQuery.length > 0) {
				newQc['$'] = {
					[MetadataModel.QcProperties.D_FIELD_COLUMN_NAME]: Entities.StorageDrivesGroups.FieldColumn.Description,
			...
			if (this._dateOfCreationFrom.length > 0) {
				newQc['$.created_on'] = {
					[MetadataModel.QcProperties.D_FIELD_COLUMN_NAME]: Entities.StorageDrivesGroups.FieldColumn.CreatedOn,
					[MetadataModel.QcProperties.D_TABLE_COLLECTION_UID]: this._metadataModelsSearch.searchmetadatamodel[MetadataModel.FgProperties.DATABASE_TABLE_COLLECTION_UID],
					[MetadataModel.QcProperties.FG_FILTER_CONDITION]: [
						[
							{
			...
			if (this._dateOfLastUpdatedOnFrom.length > 0) {
				newQc['$.last_updated_on'] = {
					[MetadataModel.QcProperties.D_FIELD_COLUMN_NAME]: Entities.StorageDrivesGroups.FieldColumn.LastUpdatedOn,
					[MetadataModel.QcProperties.D_TABLE_COLLECTION_UID]: this._metadataModelsSearch.searchmetadatamodel[MetadataModel.FgProperties.DATABASE_TABLE_COLLECTION_UID],
					[MetadataModel.QcProperties.FG_FILTER_CONDITION]: [
						[
							{
	...
	private async _handleDeleteStorageDrivesGroups(selectedDataIndexes: number[]) {
		const data = selectedDataIndexes.map((dIndex) => this._metadataModelsSearch.searchresults.data![dIndex])

		try {
			this.dispatchEvent(new CustomEvent(Lib.CustomEvents.SHOW_LOADING_SCREEN, { detail: { loading: true, loadingMessage: `Deleting/deactivating ${Entities.StorageDrivesGroups.RepositoryName}...` }, bubbles: true, composed: true }))
			if (!this._appContext.GetCurrentdirectorygroupid()) {
				return
			}
			const fetchUrl = new URL(`${Url.ApiUrlPaths.Storage.Drives.Groups}/${Url.Action.DELETE}`)
	...
	@state() private _showCreateNewStorageDrivesGroups = false
	...
	private _setupDirectoryGroupsSearchTask = new Task(this, {
		...
		args: () => [this.data?.directorygroupsid, this._appContext.appcontext?.targetjoindepth, this._showCreateNewStorageDrivesGroups, this._createGroupRuleAuthorizationsStep, this._directoryGroupsShowQueryPanel, this._directoryGroupsSearch?.searchmetadatamodel]
	})

	private _currentTargetJoinDepth: number | undefined = undefined

	@state() private _storageDrivesShowQueryPanel = false
	private _storageDrivesSearch?: IMetadataModelSearchController
	@state() _storageDrivesQueryConditions: MetadataModel.QueryConditions[] = []
	@state() private _storageDrivesFilterExcludeIndexes: number[] = []
	@state() private _selectedStorageDrivesIndexes: number[] = []
	private _setupStorageDrivesSearchTask = new Task(this, {
		task: async ([showCreate, step, showQueryPanel], { signal }) => {
			if (!showCreate || step !== 1) {
				return
			}
			Log.Log(Log.Level.DEBUG, this.localName, 'this._setupStorageDrivesSearchTask')

			if (!this._storageDrivesSearch) {
				this._storageDrivesSearch = new MetadataModelSearchController(this, `${Url.ApiUrlPaths.Storage.Drives.Url}${Url.MetadataModelSearchGetMMPath}`, `${Url.ApiUrlPaths.Storage.Drives.Url}${Url.MetadataModelSearchPath}`)
			}

			if (showQueryPanel && (!this._storageDrivesSearch.searchmetadatamodel || Object.keys(this._storageDrivesSearch.searchmetadatamodel).length == 0)) {
				await this._storageDrivesSearch.FetchMetadataModel(this._appContext.appcontext?.iamdirectorygroupid, this._appContext.appcontext?.targetjoindepth || 1, signal)
			}
		},
		args: () => [this._showCreateNewStorageDrivesGroups, this._createGroupRuleAuthorizationsStep, this._storageDrivesShowQueryPanel]
	})

	private async _handleCreateStorageDrivesGroups() {
		if (!this._directoryGroupsSearch?.searchresults.data || !this._storageDrivesSearch?.searchresults.data) {
			return
		}
		...
		let newStorageDrivesGroups: Entities.StorageDrivesGroups.Interface[] = []
		for (const dg of directoryGroups) {
			for (const sdIndex of this._selectedStorageDrivesIndexes) {
				const sd: Entities.StorageDrives.Interface = this._storageDrivesSearch.searchresults.data[sdIndex]
				if (Array.isArray(sd.id) && sd.id.length === 1) {
					newStorageDrivesGroups.push({
						directory_groups_id: dg.id,
						storage_drives_id: sd.id
					})
				}
			}
		}

		if (newStorageDrivesGroups.length === 0) {
			return
		}

		try {
			this.dispatchEvent(new CustomEvent(Lib.CustomEvents.SHOW_LOADING_SCREEN, { detail: { loading: true, loadingMessage: `Creating new ${Entities.StorageDrivesGroups.RepositoryName}...` }, bubbles: true, composed: true }))
			if (!this._appContext.GetCurrentdirectorygroupid()) {
				return
			}
			const fetchUrl = new URL(`${Url.ApiUrlPaths.Storage.Drives.Groups}/${Url.Action.CREATE}`)
			fetchUrl.searchParams.append(Url.SearchParams.DIRECTORY_GROUP_ID, this._appContext.GetCurrentdirectorygroupid()!)
			fetchUrl.searchParams.append(Url.SearchParams.AUTH_CONTEXT_DIRECTORY_GROUP_ID, this._appContext.Getauthcontextdirectorygroupid())
			if (this._appContext.appcontext?.verboseresponse) {
				fetchUrl.searchParams.append(Url.SearchParams.VERBOSE_RESPONSE, `${true}`)
			...
			Log.Log(Log.Level.DEBUG, this.localName, fetchUrl, newStorageDrivesGroups)

			const fetchResponse = await fetch(fetchUrl, {
				method: 'POST',
				credentials: 'include',
				body: JSON.stringify(newStorageDrivesGroups)
			})
		...
		if (action) {
			switch (action) {
				case Url.Action.CREATE:
					this._showCreateNewStorageDrivesGroups = true
					break
	...
	protected render(): unknown {
		return html`
			<div class="flex-1 flex flex-col rounded-md bg-white shadow-md shadow-gray-800 overflow-hidden p-2 gap-y-1">
				${(() => {
					if (this._showCreateNewStorageDrivesGroups) {
						return html`
							<header class="flex-[0.5] flex gap-x-1 z-[2]">
								<button class="btn btn-circle btn-ghost flex justify-center" @click=${() => (this._showCreateNewStorageDrivesGroups = false)}>
								...
								<div class="h-fit w-fit flex gap-x-1 text-primary self-center">Create Storage Drives Groups</div>
							</header>
							...
							<nav class="w-fit steps self-center h-fit z-[1]">
								...
								>
									<div class="break-words pl-4 pr-4">Pick storage drive(s)</div>
								</button>
								<button
								...
								>
									<div class="break-words pl-4 pr-4">Create Storage Drives Groups</div>
								</button>
							</nav>
							<main class="flex-[9] flex flex-col gap-y-1 overflow-hidden z-[1]">
								${(() => {
									switch (this._createGroupRuleAuthorizationsStep) {
										...
										case 1:
											return this._setupStorageDrivesSearchTask.render({
												pending: () => html`
													<div class="flex-1 flex flex-col justify-center items-center text-xl gap-y-5">
														<div class="flex">
															<span class="loading loading-ball loading-sm text-accent"></span>
												...
												complete: () => html`
													<section class="flex-[9] flex gap-x-1 overflow-hidden">
														${(() => {
															if (!this._storageDrivesShowQueryPanel) {
																return nothing
															}
															...
															return html`
																<div class="flex-[2] flex overflow-hidden shadow-inner shadow-gray-800 rounded-md">
																	${this._importMMQueryPanel.render({
																		...
																		complete: () => html`
																			<metadata-model-view-query-panel
																				.metadatamodel=${this._storageDrivesSearch!.searchmetadatamodel}
																				.queryconditions=${this._storageDrivesQueryConditions}
																				@metadata-model-datum-input:updatemetadatamodel=${(e: CustomEvent) => {
																					this._storageDrivesSearch!.UpdateMetadatamodel(e.detail.value)
																				}}
																				@metadata-model-view-query-panel:updatequeryconditions=${(e: CustomEvent) => {
																					this._storageDrivesQueryConditions = structuredClone(e.detail.value)
																				}}
																			></metadata-model-view-query-panel>
																		`,
																		error: (e) => {
														...
														${(() => {
															if (this._windowWidth < 1000 && this._storageDrivesShowQueryPanel) {
																return nothing
															}
															...
															return html`
																<div class="flex-[3] flex flex-col gap-y-2 overflow-hidden">
																	${(() => {
																		if (this._storageDrivesSearch!.searchmetadatamodel && this._storageDrivesSearch!.searchresults.data && this._storageDrivesSearch!.searchresults.data.length > 0) {
																			return this._importMMTableTask.render({
																				pending: () => html`
																					<div class="flex-1 flex flex-col justify-center items-center text-xl gap-y-5">
																						<div class="flex">
																				...
																				complete: () => html`
																					<div class="grow-[9] border-[1px] border-gray-400 rounded-md h-fit max-h-full max-w-full flex overflow-hidden">
																						<metadata-model-view-table
																							.metadatamodel=${this._storageDrivesSearch!.searchmetadatamodel}
																							.data=${this._storageDrivesSearch!.searchresults.data!}
																							.getmetadatamodel=${this._fieldAnyMetadataModels}
																							.filterexcludeindexes=${this._storageDrivesFilterExcludeIndexes}
																							.addselectcolumn=${true}
																							.addclickcolumn=${false}
																							.selecteddataindexes=${this._selectedStorageDrivesIndexes}
																							@metadata-model-view-table:selecteddataindexesupdate=${(e: CustomEvent) => {
																								this._selectedStorageDrivesIndexes = structuredClone(e.detail.value)
																							}}
																						></metadata-model-view-table>
																					</div>
																				`,
													...
													<div class="join md:min-w-[30%] max-md:w-full self-center">
															...
															if (this._storageDrivesSearch!.searchmetadatamodel && this._storageDrivesSearch!.searchresults.data && this._storageDrivesSearch!.searchresults.data.length > 0) {
																return html`
																	<button
																		...
																		@click=${(e: Event) => {
																			e.preventDefault()
																			this._storageDrivesFilterExcludeIndexes = structuredClone(MetadataModel.FilterData(this.queryConditions, this._storageDrivesSearch!.searchresults.data!))
																			this.dispatchEvent(new CustomEvent(Lib.CustomEvents.TOAST_NOTIFY, { detail: { toastType: Lib.ToastType.INFO, toastMessage: `${this._storageDrivesFilterExcludeIndexes.length} filtered out` }, bubbles: true, composed: true }))
																		}}
																	>
																		<div class="flex gap-x-1 self-center">
																			<!--mdi:search source: https://icon-sets.iconify.design-->
														...
														<button
															...
															@click=${async () => {
																try {
																	this.dispatchEvent(new CustomEvent(Lib.CustomEvents.SHOW_LOADING_SCREEN, { detail: { loading: true, loadingMessage: 'Searching...' }, bubbles: true, composed: true }))
																	await this._storageDrivesSearch!.Search(
																		this._storageDrivesQueryConditions,
																		this._appContext.appcontext?.iamdirectorygroupid,
																		this._appContext.GetCurrentdirectorygroupid(),
																		this._appContext.appcontext?.targetjoindepth || 1,
																		this._appContext.appcontext?.skipiffgdisabled || true,
																	...
																	window.dispatchEvent(
																		new CustomEvent(Lib.CustomEvents.TOAST_NOTIFY, {
																			detail: { toastType: Lib.ToastType.SUCCESS, toastMessage: `${Array.isArray(this._storageDrivesSearch!.searchresults.data) ? this._storageDrivesSearch!.searchresults.data.length : 0} results found` },
																			bubbles: true,
																			composed: true
																		})
																	)
														...
														<button
															...
															@click=${(e: Event) => {
																e.preventDefault()
																this._storageDrivesShowQueryPanel = !this._storageDrivesShowQueryPanel
															}}
														>
															<div class="flex gap-x-1 self-center">
																<!--mdi:filter source: https://icon-sets.iconify.design-->
															...
															${(() => {
																...
																return html`<div class="text-center text-sm font-bold text-secondary-content break-words">${this._storageDrivesShowQueryPanel ? 'Hide' : 'Show'} Filter Query Panel</div>`
															})()}
														</button>
													</div>
												`,
												...
												<section class="flex-1 flex justify-center bg-gray-300 rounded-md w-full h-full min-h-[100px]">
													<div class="flex flex-col gap-y-4 h-fit self-center">
														<button
															...
															@click=${() => {
																this._handleCreateStorageDrivesGroups()
															}}
															.disabled=${(!this.data?.directorygroupsid && this._selectedDirectoryGroupsIndexes.length === 0) || this._selectedStorageDrivesIndexes.length === 0}
														>
															Create Storage Drives Groups
														</button>
													</div>
												</section>
											`
						...
						<header class="flex-[0.5] flex flex-col gap-y-1 z-[2]">
							<section class="join w-[50%] rounded-md self-center border-[1px] border-primary p-1">
								<input
									...
									type="search"
									placeholder="Search storage drives groups..."
														...
														<div class="grow-[9] border-[1px] border-gray-400 rounded-md h-fit max-h-full max-w-full flex overflow-hidden">
															<metadata-model-view-table
																...
																@metadata-model-view-table:rowclick=${async (e: CustomEvent) => {
																	const datum = e.detail.value as Entities.StorageDrivesGroups.Interface
																	if (Array.isArray(datum.directory_groups_id) && datum.directory_groups_id.length == 1 && Array.isArray(datum.storage_drives_id) && datum.storage_drives_id.length == 1) {
																		this._handlePageNavigation(`${Url.WebsitePaths.Storage.Drives.Groups}/${datum.storage_drives_id[0]}/${datum.directory_groups_id[0]}`, `Storage Drive: ${datum.storage_drives_id[0]}/${datum.directory_groups_id[0]}`)
																	}
																}}
																.addselectcolumn=${true}
																.selecteddataindexesactions=${[
																	{
																		actionName: 'Delete/deactivate selected storage drives groups',
																		action: (selectedDataIndexes: number[]) => {
																			this._handleDeleteStorageDrivesGroups(selectedDataIndexes)
																		}
																	}
																]}
															></metadata-model-view-table>
										...
										<section class="shrink-[9] overflow-auto flex justify-center bg-gray-300 rounded-md w-full h-full min-h-[100px]">
											<div class="self-center flex-1 flex flex-col max-md:max-w-[80%] min-w-fit min-h-fit gap-y-10">
												${(() => {
													...
													return html` <div class="text-xl font-bold break-words text-center">${Url.storageDrivesGroupNavigation.description}</div> `
												})()}
												<div class="flex justify-evenly flex-wrap gap-8">
													<button class="link link-hover min-h-fit h-fit min-w-fit w-fit flex flex-col justify-center" @click=${() => (this._showCreateNewStorageDrivesGroups = true)}>
														<div class="flex gap-x-1 self-center">
														...
														${(() => {
															...
															return html`<div>Create new storage drives groups</div>`
														})()}
													</button>
													<button class="link link-hover min-h-fit h-fit min-w-fit w-fit flex flex-col justify-center" @click=${() => (this._showFilterMenu = true)}>
														...
														${(() => {
															...
															return html`<div>Search for storage drives groups</div>`
														})()}
													</button>
													<button
													...
													>
														...
														${(() => {
															...
															return html`<div>Update storage drives groups</div>`
														})()}
													</button>
													<button class="link link-hover min-h-fit h-fit min-w-fit w-fit flex flex-col justify-center" @click=${() => (this._showQueryPanel = true)}>
														...
														${(() => {
															...
															return html`<div>Delete storage drives groups</div>`
														})()}
													</button>
												</div>
											</div>
