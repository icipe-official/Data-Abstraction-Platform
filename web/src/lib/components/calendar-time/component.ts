...
class Component extends LitElement {
	...
	private _mainRelativeElement!: Element

	protected firstUpdated(_changedProperties: PropertyValues): void {
		;(async () => {
			await new Promise((resolve: (e: Element) => void) => {
				if ((this.shadowRoot as ShadowRoot).querySelector(`#main-relative-container`)) {
					resolve((this.shadowRoot as ShadowRoot).querySelector(`#main-relative-container`) as Element)
					return
				}
				...
				.then((e) => {
					this._mainRelativeElement = e
				})
				.catch((err) => {
					console.error('Get main relative item failed', err)
				})
		})()
	}
	...
	protected render(): unknown {
		return html`
			<header class="flex max-h-fit w-full ${this.roundedborder ? 'rounded-md' : ''} border-2 ${this.color === Theme.Color.PRIMARY ? 'border-primary' : this.color === Theme.Color.SECONDARY ? 'border-secondary' : this.color === Theme.Color.ACCENT ? 'border-accent' : 'border-black'}">
				${(() => {
					...
					if (this.datetimeinputformat.match('yyyy-mm-dd hh:mm') || this.datetimeinputformat.match('yyyy-mm-dd') || this.datetimeinputformat.match('yyyy-mm')) {
						tabs.push(html`
							<div class="${this.color === Theme.Color.PRIMARY ? 'bg-primary text-primary-content' : this.color === Theme.Color.SECONDARY ? 'bg-secondary text-secondary-content' : this.color === Theme.Color.ACCENT ? 'bg-accent text-accent-content' : 'bg-black text-white'} font-bold min-h-full flex">
					...
					tabs.push(html`
						<div class="flex-[0.5] flex ${this.headersbottom ? 'flex-col-reverse' : 'flex-col'} border-l-[1px] ${this.color === Theme.Color.PRIMARY ? 'border-primary' : this.color === Theme.Color.SECONDARY ? 'border-secondary' : 'border-accent'}">
							...
							<div class="h-full flex">
								...
								>
									<iconify-icon icon="mdi:calendar-plus" style="color: black;" width=${Misc.IconifySize('23')} height=${Misc.IconifySize('23')}></iconify-icon>
								</button>
							</div>
						</div>
					`)
													...
													<div class="flex w-full">
														...
														>
															<iconify-icon
																...
																style="color: ${typeof this.color === 'undefined' ? 'white' : Theme.GetColorContent(this.color)};"
											...
											return html`
												<div class="self-center sm:w-[50%] max-sm:w-full flex">
													<div class="flex-1 flex flex-col">
														<button
															class="rounded-tl-lg rounded-tr-none rounded-b-none btn ${this.color === Theme.Color.PRIMARY ? 'btn-primary' : this.color === Theme.Color.SECONDARY ? 'btn-secondary' : this.color === Theme.Color.ACCENT ? 'btn-accent' : 'bg-black text-white'} h-fit${this._hour !==
																null && this._hour >= 23
																? ' btn-disabled'
																: ''}"
														...
														<button
															class="rounded-bl-lg rounded-br-none rounded-t-none btn ${this.color === Theme.Color.PRIMARY ? 'btn-primary' : this.color === Theme.Color.SECONDARY ? 'btn-secondary' : this.color === Theme.Color.ACCENT ? 'btn-accent' : 'bg-black text-white'} h-fit${this._hour !==
																null && this._hour <= 0
																? ' btn-disabled'
																: ''}"
															@click=${() => this._handleDateTimeInput(null, null, null, `${this._hour !== null ? this._hour - 1 : -1}`, null)}
