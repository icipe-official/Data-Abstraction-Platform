...
func (n *PostrgresRepository) RepoAbstractionsReviewsUpdateOne(
	...
	if iamAuthorizationRule, err := n.RepoIamGroupAuthorizationsGetAuthorized(
		...
		authWhere = fmt.Sprintf(
			"%[1]s IN (SELECT %[2]s FROM %[3]s WHERE %[4]s = '%[5]s' AND %[6]s = '%[7]s' AND %[8]s IS NULL)",
			...
			intdoment.AbstractionsRepository().DirectoryID,           //6
			iamCredential.DirectoryID[0].String(),                    //7
			intdoment.AbstractionsRepository().DeactivatedOn,         //8
		)
	}
	if iamAuthorizationRule, err := n.RepoIamGroupAuthorizationsGetAuthorized(
		ctx,
	...
	query := fmt.Sprintf(
		"UPATE %[1]s SET %[2]s = $1 WHERE %[3]s = $2 AND %[4]s = $3 AND %[3]s IN (SELECT %[5]s.%[6]s FROM %[5]s INNER JOIN %[7]s ON %[5]s.%[8]s = %[7]s.%[9]s AND %[5]s.%[10]s IS NULL AND %[7]s.%[11]s IS NULL) AND (%[12]s)",
...
func (n *PostgresSelectQuery) AbstractionsReviewsGetSelectQuery(ctx context.Context, abstraction map[string]any, abstractionParentPath string) (*SelectQuery, error) {
	...
	if tableUid, ok := abstraction[intlibmmodel.FIELD_GROUP_PROP_DATABASE_TABLE_COLLECTION_UID].(string); ok && len(tableUid) > 0 {
	...
	if value, err := intlibmmodel.DatabaseGetColumnFields(abstraction, selectQuery.TableUid, false, false); err != nil {
	...
	if !n.startSearchDirectoryGroupID.IsNil() {
		...
		cteWhere = append(
			cteWhere,
			fmt.Sprintf(
				"(%[1]s) IN (SELECT %[2]s FROM %[3]s WHERE %[4]s IN (SELECT %[5]s FROM %[6]s))",
				intdoment.AbstractionsReviewsRepository().AbstractionsID,         //1
				intdoment.AbstractionsRepository().ID,                            //2
				intdoment.AbstractionsRepository().RepositoryName,                //3
	...
	if value, err := n.extractChildMetadataModel(abstraction, abstractionsIDJoinAbstractions); err != nil {
	...
	if value, err := n.extractChildMetadataModel(abstraction, directoryIDJoinDirectory); err != nil {
	...
	selectQuery.appendLimitOffset(abstraction)
