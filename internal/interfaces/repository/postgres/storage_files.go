...
func (n *PostrgresRepository) RepoStorageFilesDeleteOne(
	...
	if _, err := transaction.Exec(ctx, query, datum.ID[0]); err == nil {
		...
		n.logger.Log(ctx, slog.LevelDebug, query, "function", intlib.FunctionName(n.RepoStorageFilesDeleteOne))
		if _, err := transaction.Exec(ctx, query, datum.ID[0]); err == nil {
			if err := fileManagement.Delete(ctx, datum, storageDrive); err != nil {
...
func (n *PostrgresRepository) RepoStorageFilesFindOneForDeletionByID(
	...
	if !slices.Contains(columns, intdoment.StorageFilesRepository().StorageDrivesID) {
		columns = append(columns, intdoment.StorageFilesRepository().StorageDrivesID)
	}
...
func (n *PostrgresRepository) RepoStorageFilesInsertOne(
	...
	valuesToInsert := []any{datum.StorageDrivesID[0], datum.DirectoryGroupsID[0], directoryID}
	columnsToInsert := []string{intdoment.StorageFilesRepository().StorageDrivesID, intdoment.StorageFilesRepository().DirectoryGroupsID, intdoment.StorageFilesRepository().DirectoryID}
	...
	if err := fileManagement.Create(ctx, storageFile, storageDrive, file); err != nil {
		transaction.Rollback(ctx)
		return nil, err
	}

	if err := transaction.Commit(ctx); err != nil {
		if err := fileManagement.Delete(ctx, storageFile, storageDrive); err != nil {
...
func (n *PostgresSelectQuery) StorageFilesGetSelectQuery(ctx context.Context, metadataModel map[string]any, metadataModelParentPath string) (*SelectQuery, error) {
	...
	if _, ok := selectQuery.Columns.Fields[intdoment.StorageFilesRepository().StorageDrivesID][intlibmmodel.FIELD_GROUP_PROP_FIELD_GROUP_KEY].(string); ok {
		if value := n.getWhereCondition(quoteColumns, selectQuery.TableUid, "", intdoment.StorageFilesRepository().StorageDrivesID, "", PROCESS_QUERY_CONDITION_AS_SINGLE_VALUE, ""); len(value) > 0 {
			selectQuery.Where[intdoment.StorageFilesRepository().StorageDrivesID] = value
		}
	}
	...
	storageDrivesIDJoinStorageDrivesGroups := intlib.MetadataModelGenJoinKey(intdoment.StorageFilesRepository().StorageDrivesID, intdoment.StorageDrivesGroupsRepository().RepositoryName)
	if value, err := n.extractChildMetadataModel(metadataModel, storageDrivesIDJoinStorageDrivesGroups); err != nil {
		n.logger.Log(ctx, slog.LevelDebug, fmt.Sprintf("extract %s child metadata model failed, error: %v", storageDrivesIDJoinStorageDrivesGroups, err))
	} else {
		if sq, err := n.StorageDrivesGroupsGetSelectQuery(
			ctx,
			value,
			metadataModelParentPath,
		); err != nil {
			n.logger.Log(ctx, slog.LevelDebug, fmt.Sprintf("get child %s psql query failed, error: %v", storageDrivesIDJoinStorageDrivesGroups, err))
		} else {
			sq.JoinType = JOIN_INNER
			sq.JoinQuery = make([]string, 1)
			sq.JoinQuery[0] = fmt.Sprintf(
				"%[1]s = %[2]s AND %[3]s = %[4]s",
				GetJoinColumnName(sq.TableUid, intdoment.StorageDrivesGroupsRepository().StorageDrivesID, true),      //1
				GetJoinColumnName(selectQuery.TableUid, intdoment.StorageFilesRepository().StorageDrivesID, false),   //2
				GetJoinColumnName(sq.TableUid, intdoment.StorageDrivesGroupsRepository().DirectoryGroupsID, true),    //3
				GetJoinColumnName(selectQuery.TableUid, intdoment.StorageFilesRepository().DirectoryGroupsID, false), //4
			)

			selectQuery.Join[storageDrivesIDJoinStorageDrivesGroups] = sq
		}
	}
