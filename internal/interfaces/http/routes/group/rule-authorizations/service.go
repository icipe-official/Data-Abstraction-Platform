...
func (n *service) ServiceGroupRuleAuthorizationsInsertMany(
	...
	verbres.Message = fmt.Sprintf("Create %s: %d successful and %d failed", intdoment.GroupRuleAuthorizationsRepository().RepositoryName, successful, failed)
...
func (n *service) ServiceGetGroupRuleAuthorizationsPageHtml(
	...
	if partialRequest {
		...
		case intdoment.WEBSITE_HTMLTMPL_PRTL_ROUTES:
			...
			if err := websiteTemplate.WebsiteTemplateRegisterPartialFile(ctx, intdoment.WEBSITE_HTMLTMPL_ROUTES_GROUPID_IAM_GROUP_RULE_AUTHORIZATIONS_PAGE, intdoment.WEBSITE_HTMLTMPL_PRTL_ROUTESGROUPID); err != nil {
				n.logger.Log(ctx, slog.LevelError, intlib.FunctionNameAndError(n.ServiceGetGroupRuleAuthorizationsPageHtml, err).Error())
				return nil, intlib.NewError(http.StatusInternalServerError, "Parse template failed")
			}
		case intdoment.WEBSITE_HTMLTMPL_PRTL_ROUTESGROUPID:
			if baseTemplate, err := websiteTemplate.WebsiteTemplateParseFile(ctx, intdoment.WEBSITE_HTMLTMPL_ROUTES_GROUPID_IAM_GROUP_RULE_AUTHORIZATIONS_PAGE); err != nil {
				n.logger.Log(ctx, slog.LevelError, intlib.FunctionNameAndError(n.ServiceGetGroupRuleAuthorizationsPageHtml, err).Error())
				return nil, intlib.NewError(http.StatusInternalServerError, "Parse template failed")
			} else {
				if err := websiteTemplate.WebsiteTemplateSetBaseTemplate(baseTemplate); err != nil {
