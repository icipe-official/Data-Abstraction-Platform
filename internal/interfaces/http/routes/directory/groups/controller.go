...
func WebsiteRouter(webService *inthttp.WebService) *chi.Mux {
	...
	router.Get("/{id}", func(w http.ResponseWriter, r *http.Request) {
		...
		if authedIamCredential != nil && !authContextDirectoryGroupID.IsNil() {
			if directoryGroupID, err := uuid.FromString(chi.URLParam(r, "id")); err == nil && !directoryGroupID.IsNil() {
		...
		if htmlContent, err := s.ServiceGetDirectoryGroupPageHtml(ctx, webService.WebsiteTemplate, webService.OpenID, isPartialRequest, partialName, authedIamCredential, authContextDirectoryGroupID, data); err != nil {
			intlib.SendJsonErrorResponse(err, w)
		} else {
