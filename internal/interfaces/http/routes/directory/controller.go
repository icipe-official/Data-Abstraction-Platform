...
import (
	...
	intfieldanymm "github.com/icipe-official/Data-Abstraction-Platform/internal/interfaces/field_any_metadata_model"
...
func WebsiteRouter(webService *inthttp.WebService) *chi.Mux {
	...
	router.Get("/{id}", func(w http.ResponseWriter, r *http.Request) {
		...
		if htmlContent, err := s.ServiceGetDirectoryPageHtml(ctx, webService.WebsiteTemplate, webService.OpenID, isPartialRequest, partialName, authedIamCredential, authContextDirectoryGroupID, data); err != nil {
			intlib.SendJsonErrorResponse(err, w)
		} else {
	...
	router.Get("/", func(w http.ResponseWriter, r *http.Request) {
		...
		if htmlContent, err := s.ServiceGetDirectoriesPageHtml(ctx, webService.WebsiteTemplate, webService.OpenID, isPartialRequest, partialName, authedIamCredential, authContextDirectoryGroupID, data); err != nil {
			intlib.SendJsonErrorResponse(err, w)
		} else {
			intlib.WebsiteSendHTMLResponse(htmlContent, w)
		}
...
func initWebsiteService(ctx context.Context, webService *inthttp.WebService) intdomint.RouteDirectoryWebsiteService {
