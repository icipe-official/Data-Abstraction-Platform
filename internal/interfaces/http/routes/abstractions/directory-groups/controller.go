...
func WebsiteRouter(webService *inthttp.WebService) *chi.Mux {
	...
	router.Get("/{id}", func(w http.ResponseWriter, r *http.Request) {
		...
		if datumID, err := uuid.FromString(chi.URLParam(r, "id")); err == nil && !datumID.IsNil() {
			...
			mmSearch.QueryConditions[0] = intdoment.MetadataModelQueryConditions{
				"$.id": intdoment.MetadataModelQueryCondition{
					...
					DatabaseFieldColumnName:    intdoment.AbstractionsDirectoryGroupsRepository().DirectoryGroupsID,
		...
		if htmlContent, err := s.ServiceGetAbstractionsDirectoryGroupPageHtml(ctx, webService.WebsiteTemplate, webService.OpenID, isPartialRequest, partialName, authedIamCredential, authContextDirectoryGroupID, data); err != nil {
			intlib.SendJsonErrorResponse(err, w)
		} else {
	...
	router.Get("/", func(w http.ResponseWriter, r *http.Request) {
		...
		if htmlContent, err := s.ServiceGetAbstractionsDirectoryGroupsPageHtml(ctx, webService.WebsiteTemplate, webService.OpenID, isPartialRequest, partialName, authedIamCredential, authContextDirectoryGroupID, data); err != nil {
			intlib.SendJsonErrorResponse(err, w)
		} else {
			intlib.WebsiteSendHTMLResponse(htmlContent, w)
		}
...
func initWebsiteService(ctx context.Context, webService *inthttp.WebService) intdomint.RouteAbstractionsDirectoryGroupsWebsiteService {
