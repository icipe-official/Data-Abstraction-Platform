...
func ApiCoreRouter(webService *inthttp.WebService) *chi.Mux {
	...
	router.Post("/delete", func(w http.ResponseWriter, r *http.Request) {
		...
		data := make([]*intdoment.StorageDrivesGroups, 0)
		...
		if code, verbres, err := s.ServiceStorageDrivesGroupsDeleteMany(
			ctx,
			authedIamCredential,
			nil,
			...
			webService.Logger.Log(
				ctx,
				slog.LevelInfo+1,
				intlib.LogAction(intlib.LOG_ACTION_DELETE, intdoment.StorageDrivesGroupsRepository().RepositoryName),
	...
	router.Post("/update", func(w http.ResponseWriter, r *http.Request) {
		...
		if code, verbres, err := s.ServiceStorageDrivesGroupsUpdateMany(
			ctx,
			authedIamCredential,
			nil,
			authContextDirectoryGroupID,
			...
			webService.Logger.Log(
				ctx,
				slog.LevelInfo+1,
				intlib.LogAction(intlib.LOG_ACTION_UPDATE, intdoment.StorageDrivesGroupsRepository().RepositoryName),
				ctx.Value(intlib.LOG_ATTR_CTX_KEY),
				"authenicated iam credential",
				intlib.JsonStringifyMust(authedIamCredential),
				"verbose response data",
	...
	router.Post("/create", func(w http.ResponseWriter, r *http.Request) {
		...
		if code, verbres, err := s.ServiceStorageDrivesGroupsInsertMany(
			ctx,
			authedIamCredential,
			nil,
			authContextDirectoryGroupID,
			...
			webService.Logger.Log(
				ctx,
				slog.LevelInfo+1,
				intlib.LogAction(intlib.LOG_ACTION_CREATE, intdoment.StorageDrivesGroupsRepository().RepositoryName),
				ctx.Value(intlib.LOG_ATTR_CTX_KEY),
				"authenicated iam credential",
				intlib.JsonStringifyMust(authedIamCredential),
				"verbose response data",
	...
	router.Route("/search", func(searchRouter chi.Router) {
		searchRouter.Post("/", func(w http.ResponseWriter, r *http.Request) {
			...
			if mmSearch.MetadataModel == nil {
				if value, err := s.ServiceStorageDrivesGroupsGetMetadataModel(
					ctx,
			...
			if searchResults, err := s.ServiceStorageDrivesGroupsSearch(
				ctx,
				mmSearch,
				...
				webService.Logger.Log(
					...
					slog.LevelInfo+1,
					fmt.Sprintln(
						intdoment.StorageDrivesGroupsRepository().RepositoryName,
...
func WebsiteRouter(webService *inthttp.WebService) *chi.Mux {
	...
	router.Get("/{storage_drives_id}/{directory_groups_id}", func(w http.ResponseWriter, r *http.Request) {
		ctx := context.WithValue(r.Context(), intlib.LOG_ATTR_CTX_KEY, slog.Attr{Key: intlib.LogSectionAttrKey, Value: slog.StringValue(intlib.LogSectionName(r.URL.Path, webService.Env))})
		...
		if authedIamCredential != nil && !authContextDirectoryGroupID.IsNil() {
			if storageDrivesID, err := uuid.FromString(chi.URLParam(r, "storage_drives_id")); err == nil && !storageDrivesID.IsNil() {
				if directoryGroupsID, err := uuid.FromString(chi.URLParam(r, "directory_groups_id")); err == nil && !directoryGroupsID.IsNil() {
					mmSearch := new(intdoment.MetadataModelSearch)
					if value, err := s.ServiceStorageDrivesGroupsGetMetadataModel(
						ctx,
						intmmretrieve.NewMetadataModelRetrieve(webService.Logger, webService.PostgresRepository, authContextDirectoryGroupID, authedIamCredential, nil),
					...
					mmSearch.QueryConditions[0] = intdoment.MetadataModelQueryConditions{
						"$.storage_drives_id": intdoment.MetadataModelQueryCondition{
							DatabaseTableCollectionUid: tableCollectionUid,
							DatabaseFieldColumnName:    intdoment.StorageDrivesGroupsRepository().StorageDrivesID,
								...
								{
									{
										...
										Value: map[string]any{
											...
											intlibmmodel.FIELD_SELECT_PROP_VALUE: storageDrivesID.String(),
										},
									},
								},
							},
						},
						"$.directory_groups_id": intdoment.MetadataModelQueryCondition{
							DatabaseTableCollectionUid: tableCollectionUid,
							DatabaseFieldColumnName:    intdoment.StorageDrivesGroupsRepository().DirectoryGroupsID,
							FilterCondition: [][]intdoment.MetadataModelFilterCondition{
								{
									{
										Negate:    false,
										...
										Value: map[string]any{
											intlibmmodel.FIELD_SELECT_PROP_TYPE:  intlibmmodel.FIELD_SELECT_TYPE_TEXT,
											intlibmmodel.FIELD_SELECT_PROP_VALUE: directoryGroupsID.String(),
										},
									},
								},
							},
					...
					if d, err := intdoment.WebsiteAddErrorToHTMLTemplateContext(data, isPartialRequest, partialName, http.StatusBadRequest, http.StatusText(http.StatusBadRequest)); err != nil {
						intlib.SendJsonErrorResponse(intlib.NewError(http.StatusBadRequest, http.StatusText(http.StatusBadRequest)), w)
						return
					} else {
						data = d
		...
		if htmlContent, err := s.ServiceGetStorageDriveGroupPageHtml(ctx, webService.WebsiteTemplate, webService.OpenID, isPartialRequest, partialName, authedIamCredential, authContextDirectoryGroupID, data); err != nil {
			intlib.SendJsonErrorResponse(err, w)
		} else {
	...
	router.Get("/", func(w http.ResponseWriter, r *http.Request) {
		...
		if htmlContent, err := s.ServiceGetStorageDrivesGroupsPageHtml(ctx, webService.WebsiteTemplate, webService.OpenID, isPartialRequest, partialName, authedIamCredential, authContextDirectoryGroupID, data); err != nil {
			intlib.SendJsonErrorResponse(err, w)
		} else {
			intlib.WebsiteSendHTMLResponse(htmlContent, w)
		}
...
func initWebsiteService(ctx context.Context, webService *inthttp.WebService) intdomint.RouteStorageDrivesGroupsWebsiteService {
...
func initApiCoreService(ctx context.Context, webService *inthttp.WebService) intdomint.RouteStorageDrivesGroupsApiCoreService {
	if value, err := NewService(webService); err != nil {
